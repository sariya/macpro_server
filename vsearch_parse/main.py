#!/usr/bin/env python

__author__ = "Sanjeev Sariya"
__date__= "18 Aug 2015 | Feb 08 2016"
__maintainer__= "Sanjeev Sariya"
__status__= "development"
__version__="0.5"
"""
Takes in confidence, RDP output, and OTU map
Confidence -c flag
outmap -m flag
RDP output -R flag

It create a tab delimited file
Headers contain microbiome community 

"""
import sys

#
if sys.version_info < (2,7):
    print "Python 2.7+ are needed for script"
    exit(1)
##

def print_help():
    print "RDP Excel version: ",__version__
    print "python module takes in RDP output, OTU map generated by QIIME 1.9.1, confidence threshold for RDP and run name"
    print "\t\t main.py -c <confidence> -n <run_name> -m <otu-map> -r <rdp_output> -t <genus|species>"
    print "\t\t -c confidence used to filter output"
    print "\t\t\t Default confidence 0.8"
    print "\t\t -m OTU map generated by QIIME after clustering"
    print "\t\t -r RDP file output after running RDP"
    print "\t\t -t Type of input clasfction: genus|species"
    print "\t\t -n run-name, log will be generated on this name"
    print "\t\t\t default run name - def_run"
    print "A log file is created with format %Y%m%d%H%M%S.log"
    print "Copyright (c) Price Lab"
    sys.exit()
#} print_help ends

if(len(sys.argv) ==1):
    print_help()
#
import argparse
import logging

from parse_files import *
from sanity_test.test_parameters import check_inputs
#         
def main(**_temp_dict):
#{
    from time import strftime
    current_time=strftime("%Y%m%d%H%M%S")

    log_file_name=_temp_dict['run_name']+"_"+str(_temp_dict['conf'])+"_"+_temp_dict['type']+"_"+current_time+".log"
    #log_file_name=_temp_dict['run_name']+"_"+current_time+"_conf_"+str(_temp_dict['conf'])+".log"
    logging.basicConfig(filename=log_file_name,format='%(asctime)s %(message)s',datefmt='%m/%d/%Y %I:%M:%S %p',level=logging.DEBUG)
    logging.debug("Thanks for using RDP excel. Version %s" %(__version__))
     
    _temp_dict['log_file']=log_file_name
    map_file,rdp_file=check_inputs(**_temp_dict)
    
    logging.debug("Input file are correct ")
    rep_sample_map=read_otu_map_file(map_file)
    
    for i in rep_sample_map:
        #print i,(rep_sample_map[i]).rep_seq_name
        st=""
        for k in (rep_sample_map[i]).sample_count_dict:
            st=st+k+"\t"+str(((rep_sample_map[i]).sample_count_dict)[k])+"\t"
        #print st
        
        #temp = Rep_seq()
        #temp=rep_sample_map[i]
        #for key in i.sample_count_dict:
            #print key, i.sample_count_dict[key]
    
    #read_rdp_file_level(rdp_file,_temp_dict,1)
    logging.debug("Confidence used is %f" %(_temp_dict['conf']))
    microb_dict_object=read_rdp_file(rdp_file,_temp_dict['conf'])

    microb_matrix,sample_list=create_microb_matrix(rep_sample_map,microb_dict_object)
    print_microb_matrix(microb_matrix,sample_list,microb_dict_object,_temp_dict['run_name'],_temp_dict['conf'],_temp_dict['type'])

    return log_file_name

#} Function ends
if __name__=="__main__":
    
    #make a dict of arguments
    parser=argparse.ArgumentParser("description")
    parser.add_argument ('-c', '--conf', help='confidence threshold',type=float,default=0.8,required=False) # store Confidence thrshold
    parser.add_argument ('-r','--rdp_file',help='RDP out file',required=True) # store RDP output file
    parser.add_argument ('-t','--type',help='type of clasfctn',required=True) # store type of clasfctn - genus, spc  
    parser.add_argument ('-m','--otu_map',help='otu map file',required=True) # store OTU map file
    parser.add_argument ('-n','--run_name',help='run name',required=False,default="def_run") # store run name
    args_dict = vars(parser.parse_args()) # make them dict..
    if not args_dict['type'] in ['genus','species']:
        print "Type [-t] flag is case sensitive. Accepts: genus or species "
        sys.exit()
    #------ends--->
    log_file_name=main(**args_dict)

    print "Please check %s" %(log_file_name)
